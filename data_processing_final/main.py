...
def process_raw_text(raw_text):
    try:
        ...
        prompt = (f"Extract the sequence of actions and the reason of the accident described in the following text. "
                  f"The sequence of an accident should be chronological and listed in numbers 1,2,3,4 and no more! "
                  f"The reason should be one sentence explaining why the accident described in the text happened, in one line. "
                  f"Don't write anny additional sentences and words in the answer above giving sequence and reason, "
                  f"so that I am given only that what I need. Text: \"{raw_text}\"")
        response = get_openai_response(prompt, openai_api_key)
        response_lines = response.strip().split('\n')
        if len(response_lines) >= 2:
            reason = response_lines[-1].replace("Reason: ", "")
            sequence_of_actions = " ".join(response_lines[:-1])
            # sequence_of_actions = "\n".join(response_lines[1].replace("Sequence of actions: ", ""))
        else:
